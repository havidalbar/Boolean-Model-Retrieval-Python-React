...
class TestPreprocessing(unittest.TestCase):
    def test_init(self):
        ...
        self.assertEqual(preprocessing._stopword, set(stopword_file.read().split('\n')))
        stopword_file.close()
    ...
    def test_case_folding(self):
        ...
        self.assertEqual(Preprocessing.case_folding("KeCiL!123"), "kecil!123")
        self.assertEqual(Preprocessing.case_folding("BeSaR!123", lower=False), "BESAR!123")
        self.assertEqual(Preprocessing.case_folding("besar!123", lower=False), "BESAR!123")

    def test_cleaning(self):
        my_string: str = '"terima kasih", pungkasnya.sekian dari saya.'
        self.assertEqual(Preprocessing.cleaning(my_string), 'terima kasih pungkasnya sekian dari saya')
        
    def test_filtering(self):
        my_custom_stopword: List[str] = ['dan', 'atau']
        preprocessing: Preprocessing = Preprocessing(my_custom_stopword)
        my_tokens: List[str] = ['saya', 'atau', 'kamu', 'akan', 'beli', 'novel', 'dan', 'komik']
        self.assertEqual(preprocessing.filtering(my_tokens), ['saya', 'kamu', 'akan', 'beli', 'novel', 'komik'])

    def test_tokenizing(self):
        my_string: str = 'python  adalah bahasa pemrograman'
        self.assertEqual(Preprocessing.tokenizing(my_string), ['python', 'adalah', 'bahasa', 'pemrograman'])
        self.assertEqual(Preprocessing.tokenizing(my_string, splitter=' '), ['python', '', 'adalah', 'bahasa', 'pemrograman'])
        self.assertEqual(Preprocessing.tokenizing('(demam and panas)', regex_split='\\b\\s*\\b'), ['(', 'demam', 'and', 'panas', ')'])
